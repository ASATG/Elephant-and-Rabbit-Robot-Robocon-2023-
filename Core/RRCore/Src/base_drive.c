...
//	
	load_position_error_stop(1,20000);
	load_position_error_stop(2,20000);
	load_position_error_stop(3,20000);
	load_position_error_stop(4,20000);
...
//
	vfl = -v*(sin(phi + (M_PI/4.00f))) + (ROBOT_R*w);
	vfr = -v*(cos(phi + (M_PI/4.00f))) + (ROBOT_R*w);
	vbl = v*(cos(phi + (M_PI/4.00f))) + (ROBOT_R*w);
	vbr = v*(sin(phi + (M_PI/4.00f))) + (ROBOT_R*w);
...
void robot_joystick_velocity(void) //Tested for 4 wheel omni         
{
  ...
  if(stick_invert_flag == true)
	...
	//30-60-90
	if(VELOCITY_MAX_SPEED >= 	250)                                              //also called in somether function
	{
...
//	lyy = r*sinf(toRAD(theta));
  ...
  //robot_velocity_run(lxx,lyy,rxx,SMOOTH_STOP); for 3 wheel bd
	...
	robot_newvelocity_run((float)r,(float)theta,rxx,SMOOTH_STOP);   //0.5,0.15f //////Mecanum
}
...
void robot_gotoy_pid(float y_pos)		//Tested for 4 wheel omni
{
	
	float err_w=0,prev_err_w=0,w=0,kpw=7.3f,kdw=2.0f,kiw=0.0f,integral_w=0;
	double curr_y,integral,dy,prev_curr_y=0,y_res=0;
  float prev_err,err,vel,phi_angle;
	float real_x = 0, real_y = 0,x1,x2,y1,y2;
...
//	get_y_encoder_position(&curr_y);
	...
	while(1)
	{
		...
		y1 = ((position[1] + position[4])/2);
		y2 = ((position[2] + position[3])/2);
		curr_y = (y1 - y2)/2;
    ...
    integral += err;		
   	    ...
   	    if(vel > max_v)
		    ...
		    if(integral >1000)
				...
				robot_newvelocity_run(vel,0,0,SMOOTH_STOP);
