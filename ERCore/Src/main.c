...
//	    robot_position_init(50,50);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		/////////////////////////
		q = read_real_position(5);
		tick = HAL_GetTick();
		dt = tick - prev_tick;
		if(dt>=250)
		{
			lcd_print(1,10,(q-pd)*1000/dt,5);
			prev_tick = tick;
			pd = q;
		}
		...
		if (PS4_L1_PRESSED)
		{
			VELOCITY_MAX_SPEED = 50;
		}

		if (PS4_L1_RELEASED)
		{
			VELOCITY_MAX_SPEED = 220;
		}

		//	     gla_angle = Get_Yaw();


		//		 lcd_print(1,7,gla_angle,3);
		...
		if (pickmode == 1)
		{
			...
			if (HAL_GPIO_ReadPin(LIMIT_SW_4_GPIO_Port, LIMIT_SW_4_Pin) == 1)
			{
				limit_value4 = 75;
			}
			else
		...
		if (ps4_mode == 0) // RING PICKING MODE
		{
			...
			if (PS4_PAD_UP)
			{
				...
				if (HAL_GPIO_ReadPin(LIMIT_SW_5_GPIO_Port, LIMIT_SW_5_Pin) == 0)
				{
					mode = 0;
				}
				if (HAL_GPIO_ReadPin(LIMIT_SW_1_GPIO_Port, LIMIT_SW_1_Pin) == 1)
...
//					mode = 2;
//				}
//				if((HAL_GPIO_ReadPin(LIMIT_SW_3_GPIO_Port, LIMIT_SW_3_Pin))==0 && HAL_GPIO_ReadPin(LIMIT_SW_5_GPIO_Port, LIMIT_SW_5_Pin) == 0)
//				{
//					mode = 2;
//				}
				if (HAL_GPIO_ReadPin(LIMIT_SW_7_GPIO_Port, LIMIT_SW_7_Pin) == 1)
...
//				lcd_clear();
				lcd_gotoxy(2, 7);
				if (HAL_GPIO_ReadPin(LIMIT_SW_2_GPIO_Port, LIMIT_SW_2_Pin) == 0)
				{
					if (mode == 0)
					{
						limit_value2 = 60;
...
//			if (PS4_L1_PRESSED)
//			{
//				if (PS4_PAD_DOWN_PRESSED && (HAL_GPIO_ReadPin(LIMIT_SW_9_GPIO_Port, LIMIT_SW_9_Pin ) == 0))
//							{
//								st--;
//				//				if(st >= 0){
//									buffer1[0] = '6';
...
//			}
		...
		if(PS4_L2 && PS4_SHARE_PRESSED){
			stick_invert_flag = false;
			buffer1[0] = '7';
			ret = HAL_I2C_Master_Transmit(&hi2c1, 4 << 1, (uint8_t *)buffer1, 1, 100);
		}
		else if (PS4_SHARE_PRESSED)
		{
			stop = 1;
			buffer1[0] = '3';
			ret = HAL_I2C_Master_Transmit(&hi2c1, 4 << 1, (uint8_t *)buffer1, 1, 100);
		...
		if (HAL_GPIO_ReadPin(LIMIT_SW_9_GPIO_Port, LIMIT_SW_9_Pin) == 1 && stop == 1)
		{
			buffer1[0] = '4';
			ret = HAL_I2C_Master_Transmit(&hi2c1, 4 << 1, (uint8_t *)buffer1, 1, 100);
			st = 0;
